...
AContainer.prototype.open = async function(url, parent, left, top, width, height)
{
	...
	{
		//if(thisObj.option.inParent) thisObj.parent.$ele.append(thisObj.$ele);
		//else fApp.rootContainer.$ele.append(thisObj.$ele);
...
AContainer.prototype.createSplit = function(count, sizeArr, splitDir, barSize, panelClass)
{
	...
	//await afc.import('Framework/afc/library/ASplitter.js');
	
	if(!window.ASplitter)
	{
		console.error('ASplitter is not defined.');
		console.info("Check Default Load Settings. or ");
		console.info("await afc.import('Framework/afc/library/ASplitter.js');");
		return;
	}
