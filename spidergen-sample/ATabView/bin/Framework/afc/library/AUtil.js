...
AAwait.pendingProc = function(target)
{
	//console.log('AAwait.pendingProc');	
	
	if(target.count==0) target._reportDone();
};
...
AAwait.prototype.waitAllProm = function(callback)
{
	Promise.all(this.proms).then((values) => {
		this.proms = [];
		callback(values);
	});	
};
...
AAwait.prototype.reg = function(val)
{
	this.count++;
	
	//console.log(this.count + ' - reg - ' + val);	
};

AAwait.prototype.unreg = function(val)
{
	if(--this.count==0) 
	{
		//setTimeout(AAwait.pendingProc, 0, this);
		AAwait.pendingProc(this);
	}
	
	//console.log(this.count + ' - unreg - ' + val);	
};
...
AAwait.prototype.all = function(endCallback)
{
	...
	//나중에 추가된 것이 먼저 호출되어야 한다.
	else this.endCallbacks.unshift(endCallback);//.push(endCallback);
};

AAwait.prototype._reportDone = function()
{
	//console.log('_reportDone');
