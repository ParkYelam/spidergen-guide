...
AWindow.prototype.openAsDialog = async function(viewUrl, parent, width, height)
{
	//var bgOpt = '';
	...
	await this.open(viewUrl, parent, 0, 0, width, height);
};

//팝업메뉴와 같은 속성으로 윈도우를 오픈한다.
AWindow.prototype.openAsMenu = async function(viewUrl, parent, width, height)
{
	this.setWindowOption(
	{
		isModal: true,
...
AWindow.prototype.openCenter = async function(viewUrl, parent, width, height)
{
	this.setWindowOption(
    {
		isCenter: true
...
AWindow.prototype.openFull = async function(viewUrl, parent)
{
	await this.open(viewUrl, parent, 0, 0, '100%', '100%');
};
