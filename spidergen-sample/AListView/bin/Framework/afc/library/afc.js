...
afc.queryReady = function(acomp, callback)
{
	...
	//afc.qryWait.waitAllProm(callback);
	
	afc.qryWait.waitAllProm(()=>
	{
...
{
	//console.log('scriptWait count : ' + afc.scriptWait.count);
	//afc.scriptWait.all(callback);
	
	afc.scriptWait.waitAllProm(callback);
};

//----------------------------
//	async 함수 호출 대기

afc.asyncWait = new AAwait();
afc.asyncReady = function(callback)
{
	afc.asyncWait.all(callback);
	
	//afc.asyncWait.waitAllProm(callback);
};
...
afc.loadScriptWait = function(url, callback) 
{
	afc.loadWait.reg();

	var prom = new Promise(function(resolve, reject) 
	{
		afc.loadScript(url, function()
		{
			afc.loadWait.all(function()
			{
				resolve();
				if(callback) callback();
			});	

			//자신에서 count 가 0 이되면 위의 콜백 호출
			afc.loadWait.unreg();
		});
	});
...
afc.loadScript = function(url, callback, isReload)
{
	...
	var prom = new Promise(function(resolve, reject) 
	{
		...
		{
			//console.log('afc.loadScript : !! already load !! ' + url);
...
window.onunhandledrejection = function(error)
{
	if(error.reason && error.reason.stack) {
		const match = error.reason.stack.match(/:(\d*):(\d*)/),
			  lineNumber = match[1],
			  colNumber = match[2],
			  message = error.reason.message,
			  url = error.reason.stack.match(/\(([\s\S]*?):\d*/)[1];
