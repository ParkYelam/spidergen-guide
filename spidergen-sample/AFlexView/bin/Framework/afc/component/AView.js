...
AView.createView = function(item, url, owner, eventListener, skipUpdatePos, skipActiveDone, callback, turnback)
{
	...
	return new Promise(function(resolve, reject) 
	{
		...
		else 
		{
			...
			afc.loadHtml(item, url, _loadHelper); //, new RegExp(searchValue, 'g'), compIdPrefix);
		}
		...
		async function _loadHelper(retHtml)
		{
			...
			if(!isAView && url) 
			{
				...
				await afc.import( url.substring(0, url.lastIndexOf(".")) + '.js', null, AView.enforceClsLoad);	//true, 무조건 강제 로드
			}
			//-------------------------------------------------------------------------		
